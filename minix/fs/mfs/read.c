...
int fs_readwrite(void)
{
	...
	if((rip->i_mode & I_TYPE) == I_IMMEDIATE)
	{
		...
		if(rw_flag == WRITING)
		{
			...
			/* is the file going to need to be unconverted from immediate to regular? */
			if((f_size + nrbytes) > 32)
			{
				...
				if ((bp = new_block(rip, (off_t) 0)) == NULL)
					panic("bp not valid in fs_readwrite immediate growth");
...
int opportunistic;		/* if nonzero, only use cache for metadata */
{
	...
	/* support immediate files - since they don't have any blocks! */
	if((rip->i_mode & I_TYPE) == I_IMMEDIATE)
		return(NO_BLOCK);
