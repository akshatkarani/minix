...
int fs_readwrite(void)
{
  ...
  cum_io = 0;
	...
	if (((rip->i_mode & I_TYPE) == I_IMMEDIATE) && (rip->i_dev == 897)) {
		...
		if (rw_flag == WRITING) {
			if ((f_size + nrbytes) > 32) {
				if (position == 0 && nrbytes <= 32) {
					/* the file will not grow over 32 bytes */
					...
					// Copy the content of zone to immed buff array
					for (i = 0; i < f_size; i++) {
						immed_buff[i] = *(((char *) rip->i_zone) + i);
					}
					// Mark all zones as empty zone
					for (i = 0; i < V2_NR_TZONES; i++) {
						rip->i_zone[i] = NO_ZONE;
					}
					// Change inode.size to zero
					rip->i_size = 0;
					// Change update time of inode
					rip->i_update = ATIME | CTIME | MTIME;
					// Mark the inode dirty
					IN_MARKDIRTY(rip);
					// Request a new block, bp
					// Now get the pointer to a new block (this will contain the present content of the file plus the added content)
					bp = new_block(rip, (off_t) 0);

					if (bp == NULL)
						panic("error");
					// Copy the immed buff content to bp.data field
					for (i = 0; i < f_size; i++) {
						b_data(bp)[i] = immed_buff[i];
					}
					// Mark the block, bp dirty
					MARKDIRTY(bp);
					put_block(bp, PARTIAL_DATA_BLOCK);
					
					// same as after rw_chunk is called
					// Update pos and f size
					position += f_size;
					f_size = rip->i_size;
					// inode.i mode = REGULAR
					rip->i_mode = (I_REGULAR | (rip->i_mode & ALL_MODES));
					// file is no more immediate
					// is_immediate = 0;
				}
			} else {
				is_immediate = 1;
		...
		if (is_immediate == 1) {
			printf("Immediate File is being handled\n");
			// Call system read or system write function with zone pos as argument
			if (rw_flag == READING) {
